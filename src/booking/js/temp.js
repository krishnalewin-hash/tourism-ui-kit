...
`;
  ...
  s.id = 'booking-form-minimal-styles';
  s.textContent = css;
...
const ALLOWED_TYPES = ['geocode','address','establishment','(regions)','(cities)'];
function getAutocompleteTypesFromConfig() {
  ...
  const filtered = arr.filter(t => ALLOWED_TYPES.includes(t));
...
function wireAutocomplete(rootDoc){
  ...
  for (const sel of sels){
    ...
    for (const el of els){
      ...
      const acOpts = {
        ...
        types:  getAutocompleteTypesFromConfig()
      };
      ...
      ac.addListener('place_changed', () => {
        ...
        const AIRPORT_CODES = CONFIG.places.airportCodes;
        let display = '';
  ...
  setTimeout(()=>enhanceVisual(document),400);
  
  // === Helpers to keep the select visually identical to other inputs ===

// Copy the theme/computed look from a normal text input to a target control (e.g., <select>)
...
// Add/remove a class so the select text looks like a placeholder until a value is chosen
function applyPlaceholderClass(select){
  const isEmpty = !select.value;
  select.classList.toggle('is-placeholder', isEmpty);
}
  
  /* ===== Section X: Passenger Count Select (1–15, then 16+) ===== */
...
(function unifyFocusStyles(select){
        ...
        m.addedNodes && m.addedNodes.forEach(node=>{
          ...
          candidates.forEach(el=>{
            ...
            if(q && targetAttrs.includes(q)){
              ...
              enhanceVisual(document); // idempotent
              enhanceNextButtonMobile(document);
              ...
              if ((q==='pickup_location' || q==='drop-off_location') && window.google?.maps?.places) {
                try { wireAutocomplete(document); } catch(e){ /* noop */ }
              }
              if(q==='pickup_date'){
  ...
  /* ===== PAC Filter (hide vague localities/cities) =====
   Rules (config-driven):
   - Keep if it looks like a specific street address (has a number), OR
   - Keep if it contains any allow keywords (airports/hotels + extras),
   - Otherwise remove — but never reduce below filter.minKeep (fail-open). */
(function installPacFilter(){
  ...
  (function installPredictionPriority(){
    if (!CONFIG.places.priorityKeywords || window.__predPriorityObserver) return;

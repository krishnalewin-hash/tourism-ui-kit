/* ===== Time Picker Module =====
   Purpose: Custom time picker with 12/24h support, step controls, and popover UI
   Handles: Singleton time picker, input wiring, time formatting, validation
================================================= */

import { CONFIG } from './config.js';

// Initialize singleton time picker
function initSingletonTimePicker() {
  ...
  function updateDisplay() { 
    const cfg=cfgRef(); 
    const hourEl=root.querySelector('[data-part="hour"]'); 
    const minEl=root.querySelector('[data-part="minute"]'); 
    const apEl=root.querySelector('[data-part="ampm"]'); 
    let displayH=state.h; 
    if(cfg.format12) { 
      let hh=displayH%12; 
      if(hh===0) hh=12; 
      hourEl.textContent=String(hh).padStart(2,'0'); 
      if(apEl) apEl.textContent=state.h>=12?'PM':'AM'; 
    } else { 
      hourEl.textContent=String(displayH).padStart(2,'0'); 
      if(apEl) apEl.parentElement.style.display='none'; 
    } 
    minEl.textContent=String(Math.floor(state.m/(cfgRef().stepMinutes||15))*(cfgRef().stepMinutes||15)).padStart(2,'0'); 
  }
  ...
  function outside(e) { 
    if(root.contains(e.target) || e.target===state.input) return; 
    close(); 
  }
  ...
  root.addEventListener('click', e => {
    ...
    if(!act) return; 
    if(act==='hu') adjust(+1,0,false); 
    else if(act==='hd') adjust(-1,0,false); 
    else if(act==='mu') adjust(0,+1,false); 
    else if(act==='md') adjust(0,-1,false); 
    else if(act==='tu'||act==='td') adjust(0,0,true); 
    else if(act==='ok') commit(); 
    else if(act==='x') { close(); state.input?.focus(); }
  });
...
// Wire pickup time inputs to the singleton picker
function attachPickupTimePicker(rootDoc, specificEl) {
  ...
  input.addEventListener('focus', () => {
    if(window.__singletonTimePicker.suppressNextFocusOpen) {
      window.__singletonTimePicker.suppressNextFocusOpen = false;
      return;
    }
...
initSingletonTimePicker();

// Export time picker functionality
window.QuoteFormConfig = window.QuoteFormConfig || {};
window.QuoteFormConfig.attachPickupTimePicker = attachPickupTimePicker;
window.QuoteFormConfig.initSingletonTimePicker = initSingletonTimePicker;

export { attachPickupTimePicker, initSingletonTimePicker };
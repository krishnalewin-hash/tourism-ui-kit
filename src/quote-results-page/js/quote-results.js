 ...
 * - Updated with 2-person minimum requirement
 *
...
function injectStyles() {
  ...
  const DEFAULT_CONFIG = {
    ...
    // Updated pricing bands with 2-person minimum
    bands: [
      { maxMi: 50,  pricePP: 30 },  // 0–<50mi → $30 pp
      { maxMi: 60,  pricePP: 40 },  // 50–<60mi → $40 pp
      { maxMi: 70,  pricePP: 50 },  // 60–<70mi → $50 pp
      { maxMi: 999, pricePP: 80 }   // 70mi+     → $80 pp
    ],
    minPricePP: 30,                // absolute minimum per-person
    defaultPassengers: 2,          // default passengers (2-person minimum)
    MINIMUM_PASSENGERS: 2,         // 2-person minimum requirement
    MINIMUM_CHARGE_MESSAGE: "Minimum 2 passengers required. Price shown is for up to 2 people.",
    remote: {                      // keyword → surcharge per person (disabled for now)
      keywords: [],
      surchargePP: 0
    },
  ...
  function convertPricingRatesToConfig(pricingRates) {
    ...
    if (pricingRates.DISTANCE_BANDS && pricingRates.BASE_RATES) {
      ...
      if (pricingRates.REMOTE_AREAS) {
        CONFIG.remote.surchargePP = pricingRates.REMOTE_AREAS.SURCHARGE_USD || 0;
      }
  ...
  function buildCardHTML(state){
    const { pickup, dropoff, miles, seconds, pp, total, pax, effectivePax, isMinimumCharge, minimumMessage, showMap = true } = state;
  ...
  async function calculate(){
    ...
    loadMaps((err) => {
      ...
      service.getDistanceMatrix({
        ...
        // Enhanced pricing with 2-person minimum logic

...
(function() {
  ...
  async function fetchClientCatalog() {
    try {
      ...
      if (base.startsWith('../') || base.startsWith('./') || base.startsWith('/')) {
        catalogUrl = `${base}/clients/_build/${CLIENT}.json`;
      } else {
        catalogUrl = `https://cdn.jsdelivr.net/gh/${base}/clients/_build/${CLIENT}.json`;
      }
      
      console.log('[Tours][BlockC] Fetching client catalog:', catalogUrl);
      const response = await fetch(catalogUrl);
      if (!response.ok) throw new Error('HTTP ' + response.status);
      
      const clientConfig = await response.json();
      if (clientConfig.TOUR_CATALOG && Array.isArray(clientConfig.TOUR_CATALOG)) {
        console.log('[Tours][BlockC] Using client catalog with', clientConfig.TOUR_CATALOG.length, 'tours');
        return { tours: clientConfig.TOUR_CATALOG, _source: 'catalog' };
      }
      
      return null;
    } catch (error) {
      console.warn('[Tours][BlockC] Client catalog fetch failed:', error);
      return null;
    }
  }
  ...
  async function fetchAllTours() {
    ...
    try {
      ...
      if (tours.length > 0) {
        cachePut(url, json);
        console.log('[Tours][BlockC] Using API tours:', tours.length);
        const version = json.version || '';
        return { version, tours, _source: 'net' };
      } else {
        console.log('[Tours][BlockC] API returned empty tours array, trying catalog fallback');
      }
    } catch (e) {
      console.warn('[Tours][BlockC] API fetch error:', e, '- trying catalog fallback');
    }

    // 3) Fallback to client catalog
    const catalogResult = await fetchClientCatalog();
    if (catalogResult) {
      return catalogResult;
    }

    console.error('[Tours][BlockC] No tour data available from API or catalog');
    return null;
  }
  ...
  function getRelatedTours(currentTour, allTours, maxResults = 3) {
    ...
    console.log('[Tours][BlockC] Found', related.length, 'related tours for type:', currentType);
    return related;
  }
  ...
  function render(currentTour, allTours) {
    ...
    if (relatedTours.length === 0) {
      ...
      console.log('[Tours][BlockC] No related tours found, hiding section');
    ...
    if (sectionTitle && currentTour.type) {
      sectionTitle.textContent = `More ${currentTour.type}`;
    }
    ...
    console.log('[Tours][BlockC] Rendered', relatedTours.length, 'related tours');
  }
